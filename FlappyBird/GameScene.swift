...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        // アイテムの画像を読み込む
        let wallTexture = SKTexture(imageNamed: "coin")
        wallTexture.filteringMode = .linear
        ...
        let createWallAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let wall = SKNode()
            wall.position = CGPoint(x: self.frame.size.width + wallTexture.size().width / 2, y:self.frame.size.height * 0.2)
            wall.zPosition = 0 // 雲より手前、地面より奥
